    ...
    <div v-else>
      ...
      <div class="mb-8">
        <div class="flex flex-wrap items-stretch gap-4 mb-2">
          ...
          <div class="stat-card flex flex-col justify-between border border-blue-300 bg-white p-3 min-w-[110px] md:min-w-[140px] lg:min-w-0 flex-1">
            <p class="text-xs text-gray-600 mb-1">Total Shop Owners</p>
            <h3 class="text-xl md:text-3xl font-bold text-spnr-blue-900">{{ stats.total_owners || 0 }}</h3>
            <p class="text-[10px] text-gray-500 mt-1 md:mt-2">&nbsp;</p>
          </div>
          <div class="stat-card flex flex-col justify-between border border-blue-300 bg-white p-3 min-w-[110px] md:min-w-[140px] lg:min-w-0 flex-1">
            <p class="text-xs text-gray-600 mb-1">{{ periodLabel }} Orders</p>
            <h3 class="text-xl md:text-3xl font-bold text-spnr-blue-900">{{ stats.period_orders || 0 }}</h3>
            <p class="text-[10px] text-gray-500 mt-1 md:mt-2">Total: {{ stats.total_orders || 0 }}</p>
          </div>
          <div class="stat-card flex flex-col justify-between border border-blue-300 bg-white p-3 min-w-[110px] md:min-w-[140px] lg:min-w-0 flex-1">
          ...
          <div class="stat-card flex flex-col justify-between border border-blue-300 bg-white p-3 min-w-[110px] md:min-w-[140px] lg:min-w-0 flex-1">
            ...
            <h3 class="text-xl md:text-3xl font-bold text-green-600">₱{{ (stats.period_revenue || 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</h3>
      ...
      <div class="card mb-8 overflow-hidden border border-blue-300">
        <div class="flex justify-between items-center mb-6">
          ...
          <div class="flex items-center gap-4">
            <div class="flex items-center gap-2">
              <div class="w-3 h-3 rounded-full bg-gradient-to-r from-green-600 to-green-400"></div>
              <span class="text-sm text-gray-600">Platform Revenue (10%)</span>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-3 h-3 rounded-full bg-spnr-blue-600"></div>
            ...
            <div v-for="(item, index) in chartData" :key="item.date || item.month" class="flex-1 flex flex-col items-center justify-end group">
              ...
              <div class="relative w-full flex flex-col items-center justify-end">
                <div class="absolute -top-8 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-gray-800 text-white text-xs px-2 py-1 rounded whitespace-nowrap z-10">
                  ...
                  <div class="text-blue-400">{{ item.orders }} orders</div>
                </div>
                <div class="w-full bg-gray-200 rounded-t-lg overflow-hidden transition-all duration-500 hover:shadow-lg relative" 
                  ...
                  <div class="w-full h-full bg-gradient-to-t from-green-600 via-green-500 to-green-400 relative">
                    <div v-if="item.orders > 0 && parseFloat(item.revenue || 0) > 0" class="absolute top-1 left-1/2 transform -translate-x-1/2">
                      <div class="w-10 h-10 bg-spnr-blue-600 rounded-full flex items-center justify-center text-white text- s font-bold shadow-lg">
                        {{ item.orders }}
                      </div>
                    </div>
                  </div>
        ...
        <div class="mt-6 pt-6 border-t border-gray-200" :class="selectedPeriod === 'daily' ? 'grid grid-cols-2 gap-4' : 'grid grid-cols-3 gap-4'">
          <div class="text-center">
            <p class="text-sm text-gray-600 mb-1">Platform Revenue (10%)</p>
            <p class="text-xl font-bold text-green-600">₱{{ totalChartRevenue.toLocaleString('en-PH', { minimumFractionDigits: 2 }) }}</p>
          </div>
          <div class="text-center">
            <p class="text-sm text-gray-600 mb-1">Total Orders</p>
            <p class="text-xl font-bold text-spnr-blue-600">{{ totalChartOrders }}</p>
          </div>
      ...
      <!-- Top Shops -->
      <div class="card mb-8 overflow-hidden border border-blue-300">
        <h2 class="text-xl font-semibold text-gray-800 mb-4">Top Performing Shops</h2>
        <div>
          <!-- Desktop table -->
            ...
            <table class="min-w-full">
              <thead>
                <tr class="border-b border-gray-200">
                  <th class="text-left py-3 px-4 text-sm font-semibold text-gray-700">Shop</th>
                  <th class="text-right py-3 px-4 text-sm font-semibold text-gray-700">Orders</th>
                  <th class="text-right py-3 px-4 text-sm font-semibold text-gray-700">Admin Revenue (10%)</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="shop in stats.top_shops" :key="shop.shop_id" class="border-b border-gray-100">
                  <td class="py-3 px-4 text-sm text-gray-800">{{ shop.shop?.shop_name || 'Unknown' }}</td>
                  <td class="py-3 px-4 text-sm text-right font-semibold text-gray-800">{{ shop.order_count }}</td>
                  <td class="py-3 px-4 text-sm text-right font-semibold text-green-600">₱{{ parseFloat(shop.admin_revenue || 0).toFixed(2) }}</td>
                </tr>
                <tr v-if="!stats.top_shops || stats.top_shops.length === 0">
          ...
          <div class="sm:hidden space-y-3">
            <div v-for="shop in stats.top_shops" :key="shop.shop_id" class="card bg-gray-50 border border-blue-200 p-3">
              <div v-if="compactMobile" class="flex items-center justify-between">
                <div>
                  <div class="text-sm font-semibold text-gray-800">{{ shop.shop?.shop_name || 'Unknown' }}</div>
                  <div class="text-xs text-gray-500">{{ shop.order_count }} orders</div>
                </div>
                <div class="text-right text-sm font-semibold text-green-600">₱{{ parseFloat(shop.admin_revenue || 0).toFixed(2) }}</div>
              </div>
              <div v-else class="flex flex-col gap-2">
                <div class="flex justify-between items-center">
                  <div>
                    <h3 class="text-lg font-semibold text-gray-800">{{ shop.shop?.shop_name || 'Unknown' }}</h3>
                    <p class="text-sm text-gray-600">{{ shop.order_count }} orders</p>
                  </div>
                  <div class="text-right text-green-600 font-bold">₱{{ parseFloat(shop.admin_revenue || 0).toFixed(2) }}</div>
                </div>
              </div>
            </div>
            <div v-if="!stats.top_shops || stats.top_shops.length === 0" class="py-8 text-center text-gray-500">No data available</div>
          </div>
        </div>
      </div>

      <!-- Revenue Breakdown by Shop -->
      <div v-if="stats.shop_revenues && stats.shop_revenues.length > 0">
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Revenue Breakdown by Shop</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div v-for="shop in stats.shop_revenues" :key="shop.shop_id" class="card overflow-hidden border border-blue-300">
            <h3 class="text-lg font-semibold text-spnr-blue-900 mb-3">{{ shop.shop?.shop_name || 'Unknown Shop' }}</h3>
            <div class="space-y-2">
              <div class="flex justify-between items-center">
                <span class="text-sm text-gray-600">Total Sales:</span>
                <span class="text-sm font-semibold text-gray-800">₱{{ parseFloat(shop.total).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-sm text-gray-600">Shop Revenue (90%):</span>
                <span class="text-sm font-semibold text-blue-600">₱{{ parseFloat(shop.shop_earnings).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
              </div>
              <div class="flex justify-between items-center border-t pt-2">
                <span class="text-sm font-semibold text-gray-700">Admin Revenue (10%):</span>
                <span class="text-sm font-bold text-green-600">₱{{ parseFloat(shop.admin_earnings).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</span>
              </div>
            </div>
          </div>
        </div>
...
const chartTitle = computed(() => {
  const titles = {
    daily: 'Today\'s Revenue Performance',
    weekly: 'Weekly Revenue (Sunday - Saturday)',
    monthly: 'Monthly Revenue (Current Month)',
    yearly: 'Monthly Revenue Overview (2025)'
  }
  return titles[selectedPeriod.value] || 'Revenue Overview'
})
...
const downloadReport = async () => {
  try {
    const response = await api.get('/admin/download-report', {
      params: { period: selectedPeriod.value },
      responseType: 'blob'
    })
    ...
    link.href = url
    link.setAttribute('download', `admin-revenue-report-${selectedPeriod.value}-${new Date().toISOString().split('T')[0]}.pdf`)
