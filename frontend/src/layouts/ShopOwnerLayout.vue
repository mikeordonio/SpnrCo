<template>
  <div :class="['min-h-screen', { 'shop-compact': compactMobile }]">
    <!-- Navigation -->
    <nav class="navbar sticky top-0 z-50">
      <div class="w-full px-6 lg:px-12 xl:px-16">
        <div class="flex justify-between items-center h-20">
          ...
          <div class="hidden lg:flex items-center space-x-3 xl:space-x-4">
            ...
            <div class="border-l border-blue-300 pl-4 ml-4 flex items-center space-x-3">
              ...
              <div class="relative">
                <button
                  @click="shopSettingsOpen = !shopSettingsOpen"
                  class="nav-btn flex items-center"
                  :class="{ 'nav-btn-active': $route.path.startsWith('/shop/settings') || $route.path.startsWith('/shop/security') || $route.path.startsWith('/shop/profile') }"
                >
                  {{ authStore.user?.name }}
                  <svg class="w-4 h-4 ml-1 transition-transform" :class="{ 'rotate-180': shopSettingsOpen }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                  </svg>
                </button>
                <div
                  v-if="shopSettingsOpen"
                  class="absolute left-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 z-50"
                  @click="shopSettingsOpen = false"
                >
                  <router-link
                    to="/shop/settings"
                  ...
                  <router-link
                    to="/shop/profile"
                    class="block px-4 py-3 text-sm text-gray-700 hover:bg-spnr-blue-50 hover:text-spnr-blue-900 transition-colors border-t border-gray-100"
                  >
                    Profile Information
                  </router-link>
                  <router-link
        ...
        <div v-if="mobileMenuOpen" class="mobile-menu lg:hidden border-t border-blue-200 py-2">
          <router-link to="/shop/dashboard" @click="mobileMenuOpen = false" class="block px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:bg-spnr-blue-50 hover:text-spnr-blue-900 font-medium transition-all">Dashboard</router-link>
          <router-link to="/shop/services" @click="mobileMenuOpen = false" class="block px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:bg-spnr-blue-50 hover:text-spnr-blue-900 font-medium transition-all">Services</router-link>
          <router-link to="/shop/orders" @click="mobileMenuOpen = false" class="block px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:bg-spnr-blue-50 hover:text-spnr-blue-900 font-medium transition-all">Orders</router-link>
          <div class="border-t border-blue-200 mt-2 pt-2">
            ...
            <router-link to="/shop/settings" @click="mobileMenuOpen = false" class="block px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:bg-spnr-blue-50 hover:text-spnr-blue-900 font-medium transition-all">Shop Information</router-link>
            <router-link to="/shop/profile" @click="mobileMenuOpen = false" class="block px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:bg-spnr-blue-50 hover:text-spnr-blue-900 font-medium transition-all">Profile Information</router-link>
            <router-link to="/shop/security" @click="mobileMenuOpen = false" class="block px-4 py-2.5 sm:py-3 text-sm sm:text-base text-gray-700 hover:bg-spnr-blue-50 hover:text-spnr-blue-900 font-medium transition-all">Change Password</router-link>
          </div>
          <div class="border-t border-blue-200 mt-2 pt-2">
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto">
          <div v-if="notifications.length === 0" class="p-6 sm:p-8 text-center text-gray-500">
  ...
  try {
    ...
    await Promise.all(
      unread.map(notification => 
        api.put(`/shop/notifications/${notification.id}/read`)
      )
    )
    // Update local state
...
const deleteNotification = async (id) => {
  try {
    await api.delete(`/shop/notifications/${id}`)
  ...
  try {
    ...
    await Promise.all(
      notifications.value.map(notification => 
        api.delete(`/shop/notifications/${notification.id}`)
      )
    )
    notifications.value = []
